                        ...
                        .map((item, index) =>
                            <Link key={index} href={`area/kampus/${Generateslug(item.name)}`}>
                                <div className="flex hover:text-indigo-700 py-1 align-middle text-gray-900 items-center cursor-pointer">
                                    <div class="m-1 mr-2 px-3 relative flex justify-center items-center rounded-lg hover:bg-gray-500 bg-indigo-700 text-xl text-white uppercase" style={{ width: '40px', height: '40px' }}>{TitleCase(item.name)}</div>
